...
export const Tool_Catalog: React.FC = () => {
    const [bomDatabase, setBomDatabase] = useState<Record<string, { descricao: string, codigo: string, quantidade: string }[]> | null>(null);
    ...
    useEffect(() => {
        ...
        fetch('/data/bom_database.json')
            ...
            .catch(err => console.error("Erro ao carregar banco BOM:", err));
    }, []);
    ...
    const fetchData = async () => {
        ...
        const bomData = bomDatabase ? bomDatabase[selectedModel] : null;
        if (bomData) {
        ...
        <div className="animate-fadeIn pb-24">
            ...
            <Card className="mb-6">
                <Select value={selectedModel} onChange={(e) => setSelectedModel(e.target.value)} label="SELECIONE O MODELO EXATO (LISTA OFICIAL)">
                    {bomDatabase ? Object.keys(bomDatabase).map(model => (
