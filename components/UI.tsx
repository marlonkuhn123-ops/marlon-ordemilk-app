...
export const Input: React.FC<React.InputHTMLAttributes<HTMLInputElement> & { label?: string }> = ({ label, className = '', ...props }) => {
    return (
        <div className="mb-4 w-full group">
            {label && <label className="block text-[10px] font-black uppercase tracking-wider mb-1.5 ml-1 transition-colors text-white/70 group-focus-within:text-[#1abc9c]">
                {label}
            </label>}
            <input 
...
export const AIOutputBox: React.FC<{ content: string; isLoading: boolean; title?: string }> = ({ content, isLoading, title = "ANÁLISE DO SISTEMA" }) => {
    ...
    return (
        <div className="mt-6 rounded-lg overflow-hidden shadow-xl animate-slide-up border bg-[#1a1a1a] border-[#3d3d3d] shadow-black/50">
            {/* CABEÇALHO */}
            <div className="px-4 py-2.5 flex justify-between items-center border-b bg-[#242424] border-[#3d3d3d]">
                <div className="flex items-center gap-2">
                    <div className="w-1.5 h-1.5 rounded-full animate-pulse bg-[#1abc9c]"></div>
                    <h3 className="text-[9px] font-black uppercase tracking-widest text-[#1abc9c] truncate max-w-[150px]">{title}</h3>
                </div>
                <button 
                    onClick={handleCopy}
            ...
            {/* CONTEÚDO */}
            <div className="p-4 text-sm leading-relaxed text-xs text-gray-300 break-words">
                {formattedContent}
            </div>

            {/* RODAPÉ */}
            <div className="p-3 border-t border-[#3d3d3d] bg-[#242424]">
                 <button 
                    onClick={shareWhatsapp}
                    className="w-full py-3 rounded-lg font-bold text-[10px] uppercase tracking-widest bg-[#1abc9c] text-white shadow-lg hover:bg-[#16a085] hover:scale-[0.99] transition-all flex items-center justify-center gap-2"
                >
